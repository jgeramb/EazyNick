...
public class OutgoingPacketInjector_1_7 {
	...
	public void init() {
		...
		reflectionHelper.getFirstFieldByType(reflectionHelper.getNMSClass("NetworkManager"), Channel.class).ifPresent(field -> {
						...
						try {
							if (msg.getClass().getSimpleName().equals("PacketPlayOutNamedEntitySpawn")) {
								...
								if(playerObject != null) {
									...
									if(utils.getSoonNickedPlayers().contains(uuid) && setupYamlFile.getConfiguration().getBoolean("SeeNickSelf") && (reflectionHelper.getField(msg.getClass(), "action").getInt(msg) == 0))
										return;
								...
								Object editedComponent = replaceNames(reflectionHelper.getField(msg.getClass(), "a").get(msg));
	...
	private Object replaceNames(Object iChatBaseComponent) {
		...
		try {
			if(iChatBaseComponent != null) {
				...
				if(!(fullText.toString().contains(ChatColor.stripColor(utils.getLastChatMessage())) || fullText.toString().startsWith(ChatColor.stripColor(utils.getPrefix())))) {

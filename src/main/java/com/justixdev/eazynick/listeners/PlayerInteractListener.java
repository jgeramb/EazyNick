...
public class PlayerInteractListener implements Listener {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if ((event.getItem() != null) && (event.getItem().getType() != Material.AIR) && (event.getItem().getItemMeta() != null) && event.getItem().getItemMeta().hasDisplayName()) {
			...
			if (setupYamlFile.getConfiguration().getBoolean("NickItem.getOnJoin") && player.hasPermission("eazynick.item")) {
				if (event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
					if(!(eazyNick.getVersion().startsWith("1_8") || eazyNick.getVersion().startsWith("1_7"))) {

...
public class PlayerJoinListener implements Listener {
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		if(!(isAPIMode)) {
			if(setupYamlFile.getConfiguration().getBoolean("OverwriteJoinQuitMessages") && ((setupYamlFile.getConfiguration().getBoolean("BungeeCord") && mysqlNickManager.isPlayerNicked(uniqueId)) || utils.getLastNickData().containsKey(uniqueId))) {
				...
				} else if(setupYamlFile.getConfiguration().getBoolean("SaveLocalNickDatas") && savedNickDataYamlFile.getConfiguration().contains(uniqueIdString))
					message = message.replace("%name%", savedNickDataYamlFile.getConfigString(uniqueIdString + ".nick_name")).replace("%displayName%", savedNickDataYamlFile.getConfigString(uniqueIdString + ".chat_prefix") + savedNickDataYamlFile.getConfigString(player, uniqueIdString + ".nick_name") + savedNickDataYamlFile.getConfigString(player, uniqueIdString + ".chat_suffix"));
				...
				if (setupYamlFile.getConfiguration().getBoolean("BungeeCord") && (!(setupYamlFile.getConfiguration().getBoolean("LobbyMode")) || (player.hasPermission("eazynick.bypasslobbymode") && setupYamlFile.getConfiguration().getBoolean("EnableBypassLobbyModePermission"))) && mysqlNickManager.isPlayerNicked(uniqueId)) {
					...
					event.setJoinMessage(event.getJoinMessage().replace(player.getName(), utils.getLastNickData().get(uniqueId).getNickName()));
				} else if (setupYamlFile.getConfiguration().getBoolean("SaveLocalNickDatas") && savedNickDataYamlFile.getConfiguration().contains(uniqueIdString)) {
					if (event.getJoinMessage().contains("formerly known as"))
						event.setJoinMessage("§e" + player.getName() + " joined the game");
	
					event.setJoinMessage(event.getJoinMessage().replace(player.getName(), savedNickDataYamlFile.getConfigString(uniqueIdString + ".nick_name")));
				}
			}
		}
						...
						if (setupYamlFile.getConfiguration().getBoolean("BungeeCord")) {
							...
							if (!(setupYamlFile.getConfiguration().getBoolean("DisconnectUnnick"))) {
								if(utils.getLastNickData().containsKey(uniqueId)) {
									...
									Bukkit.getPluginManager().callEvent(new PlayerNickEvent(player, savedNickDataYamlFile.getConfigString(uniqueIdString + ".nick_name"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".skin_name"), UUID.fromString(savedNickDataYamlFile.getConfigString(uniqueIdString + ".spoofed_uniqueid")), savedNickDataYamlFile.getConfigString(uniqueIdString + ".chat_prefix"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".chat_suffix"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".tablist_prefix"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".tablist_suffix"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".nametag_prefix"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".nametag_suffix"), false, true, savedNickDataYamlFile.getConfiguration().getInt(uniqueIdString + ".sort_id"), savedNickDataYamlFile.getConfigString(uniqueIdString + ".group_name")));
							}
						}

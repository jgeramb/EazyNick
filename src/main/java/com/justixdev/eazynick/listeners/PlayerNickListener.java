...
public class PlayerNickListener implements Listener {
	...
	public void onPlayerNick(PlayerNickEvent event) {
		...
		if(!(event.isCancelled())) {
			...
			utils.getCanUseNick().put(player.getUniqueId(), System.currentTimeMillis() + (setupYamlFile.getConfiguration().getLong("Settings.NickDelay") * 1000L));
			...
			} else if(setupYamlFile.getConfiguration().getBoolean("SaveLocalNickData") && !(setupYamlFile.getConfiguration().getBoolean("DisconnectUnnick"))) {

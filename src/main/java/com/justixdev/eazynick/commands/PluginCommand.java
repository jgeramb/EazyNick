				...
				if(args[0].equalsIgnoreCase("2")) {
					...
					if(utils.isSupportMode()) {
						...
						if(sender instanceof Player)
							((Player) sender).spigot().sendMessage(new ComponentBuilder(prefix + "§71. Join the discord server §8(discord.justix-dev.com)").event(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://discord.justix-dev.com/")).event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Collections.singletonList(new Text("§7Click here to open the discord invitation link")))).create());
						else

				...
				if(args.length >= 1) {
					...
					if(t != null) {
						if(Utils.nickedPlayers.contains(p.getUniqueId())) {
							p.sendMessage(Utils.PREFIX + ChatColor.translateAlternateColorCodes('&', LanguageFileUtils.cfg.getString("Messages.RealName").replace("%realName%", UUIDFetcher_1_8_R1.getName(t.getUniqueId()))));
						} else {

...
public class NickManager_1_8_R1 {
		...
		try {
			...
			for(Player all : Bukkit.getOnlinePlayers()) {
				...
				if(cpAll.canSee(cp)) {
					if(!(cpAll.getUniqueId().equals(cp.getUniqueId()))) {
						if(!(all.hasPermission("nick.bypass"))) {
							sendPacket(cpAll, new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.UPDATE_DISPLAY_NAME, new EntityPlayer[] { cp.getHandle() }));
						}
					} else {
						if(FileUtils.cfg.getBoolean("SeeNickSelf") == true) {
							sendPacket(cp, new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.UPDATE_DISPLAY_NAME, new EntityPlayer[] { cp.getHandle() }));
						}
					}
				}
			}

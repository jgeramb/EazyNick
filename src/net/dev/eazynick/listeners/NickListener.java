		...
		Bukkit.getScheduler().runTaskLater(EazyNick.getInstance(), new Runnable() {
			...
			public void run() {
				if(p.hasPermission("nick.bypass")) {
					if((EazyNick.mysql != null) && EazyNick.mysql.isConnected()) {
				...
				if (e.getView().getTitle().equalsIgnoreCase(LanguageFileUtils.getConfigString("NickGUI.InventoryTitle"))) {
					...
					if (e.getView().getTitle().equalsIgnoreCase(inventoryName)) {
						...
						if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase(LanguageFileUtils.getConfigString("NickNameGUI.BackItem.DisplayName"))) {
							...
							if (e.getCurrentItem().getType().equals(Material.getMaterial((EazyNick.version.startsWith("1_13") || EazyNick.version.startsWith("1_14") || EazyNick.version.startsWith("1_15")) ? "PLAYER_HEAD" : "SKULL_ITEM"))) {
								String nickName = "";
